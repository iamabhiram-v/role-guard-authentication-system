...
export class UploadController {
  ...
  async uploadWorkspaceIcon(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const file = (req as any).file;
      if (!file) throw new BadRequestError('No file provided');
      if (!ALLOWED_TYPES.includes(file.mimetype)) throw new BadRequestError('Only JPEG, PNG, or WebP images allowed');
      ...
      const userId = req.user!.userId;
      // updateWorkspace already enforces owner/admin role internally.
      const workspace = await workspaceService.updateWorkspace(workspaceId, userId, {
        iconUrl: result.data!.url,
      });

      res.status(200).json({ status: 'success', data: { iconUrl: result.data!.url, workspace } });
    } catch (err) {
      next(err);
    }
  }
